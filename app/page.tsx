...
export default function Home() {
  const { data: session } = useSession(); // Adicionado para controle de permissão
  ...
  // Preenche o nome do técnico automaticamente com o usuário logado
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white font-sans p-6">
      <header className="max-w-4xl mx-auto mb-10 text-center border-b border-gray-800 pb-6 relative">
        <button 
          onClick={() => signOut({ callbackUrl: '/login' })}
          className="absolute right-0 top-0 bg-red-900/20 hover:bg-red-600 text-red-500 hover:text-white px-3 py-1 rounded-lg text-xs font-bold transition-all border border-red-900/50"
        >
          SAIR
        </button>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full bg-gray-800 border border-gray-700 rounded-lg p-3 text-white focus:border-blue-500 focus:outline-none transition-colors ${!!session?.user?.name ? 'opacity-60 cursor-not-allowed' : ''}`}
          ...
          {loadingData ? (
            ...
            <div className="space-y-3 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
              {tickets.map((t, i) => {
                ...
                const nivel = buscarValor(t, ['NIVEL', 'ANDAR']);
                ...
                return (
                  <div key={i} className={`relative overflow-hidden rounded-xl border-l-[8px] shadow-lg ${corCSS} p-4 flex justify-between items-start transition-transform hover:scale-[1.02]`}>
                    ...
                    {/* --- ÁREA DE STATUS E CANCELAMENTO --- */}
                    <div className="text-right flex flex-col items-end gap-1">
                      <span className={`text-[10px] font-bold uppercase px-2 py-1 rounded text-white bg-black/40 border border-white/20`}>
                        {statusText}
                      </span>
                      
                      {/* Lógica de Cancelamento para Admin Rackcontrol */}
                      {(session?.user as any)?.role === "ADMIN_RACKCONTROL" && isAguardando && (
                        <button 
                          onClick={() => {
                            const justificativa = prompt("Justificativa para o cancelamento:");
                            if (justificativa) {
                              fetch(`/api/racks/${t.id}`, {
                                method: "PATCH",
                                headers: { "Content-Type": "application/json" },
                                body: JSON.stringify({ action: 'cancelar', justificativa })
                              }).then(() => fetchTickets());
                            }
                          }}
                          className="mt-2 text-[10px] bg-red-900/20 text-red-500 border border-red-900/50 px-2 py-1 rounded hover:bg-red-600 hover:text-white transition-all font-bold uppercase"
                        >
                          CANCELAR
                        </button>
                      )}
